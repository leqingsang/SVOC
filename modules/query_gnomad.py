...
def query_gnomAD_local(chrom, pos, ref, alt, vcf_file):
    ...
    # """
    query_cmd = (
        f"bcftools view -r {chrom}:{pos}-{pos} {vcf_file} | "
        f"bcftools query -f '%CHROM %POS %REF %ALT %INFO/controls_popmax %INFO/controls_AC_popmax %INFO/controls_AN_popmax %INFO/controls_AF_popmax\n' | "
        f"awk '$1==\"{chrom}\" && $2=={pos} && $3==\"{ref}\" && $4==\"{alt}\"'"
    )
    try:
...
if __name__ == "__main__":
    vcf_file = "/mnt/svoc/svocdb/gnomadv2/gnomad.exomes.r2.1.1.sites.vcf.bgz"
    chrom = "17"
    pos = 7577046
    ref = "C"
    alt = "T"

    result = query_gnomAD_local(chrom, pos, ref, alt, vcf_file)
    if result:
        print("Result:", result)
    else:
        print("No matching variant found.")

async def query_gnomAD_api_genome(gene, chr, pos, ref, alt, cDNA):
...
async def query_gnomAD_api_exome(gene, chr, pos, ref, alt, cDNA):
